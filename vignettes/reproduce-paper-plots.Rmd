---
title: "Reproduce paper plots with powerScPop"
...
for(r in readDepths){
  for(c in cellCounts){
    count.est<-powerScPop::estimate.exp.prob.param(nSamples=measuredIndivs,
                                                   readDepth=r,nCellsCt=c,
                                                   powerScPop::read.umi.fit,
                                                   powerScPop::gamma.mixed.fits,
                                                   powerScPop::gamma.probs,
                                                   ct=ct,
                                                   powerScPop::disp.fun.param,
                                                   nGenes=nGenes,
...
eqtl.ranks<-powerScPop::eqtl.ref.study
de.ranks<-powerScPop::de.ref.study[de.ref.study$type=="PBMC",]
de.rank.lung<-powerScPop::de.ref.study[de.ref.study$type=="lung",]
de.ranks.panc<-powerScPop::de.ref.study[de.ref.study$type=="pancreas",]
...
cellsPerPerson<-c(100,200,500,700,seq(1000,12000,500))
totalBudget<-10000

power.study<-optimize.constant.budget(totalBudget,readDepth,cellsPerPerson,costKit,
                              costFlowCell,readsPerFlowcell,ct.freq,comp.type,
                              de.ref.study,ref.study.name,personsPerLane,
                              read.umi.fit,gamma.mixed.fits,gamma.probs,
                              ct,disp.fun.param,
...
g.rd<-g.rd + scale_x_continuous("Cells per person",sec.axis=sec_axis(~., breaks=labels,
                                                                    labels=sapply(labels,function(c)floor(sampleSizeBudgetCalculation(c,max.study$readDepth,
    totalBudget, costKit, personsPerLane, costFlowCell,readsPerFlowcell))),                                                              name="Sample size"))+               
...
g.cp <- g.cp + scale_x_continuous("Read depth",sec.axis=sec_axis(~., breaks=labels,
    ...
    c,totalBudget, costKit, personsPerLane, costFlowCell,readsPerFlowcell))),                                                              name="Sample size"))+
...
power.study<-optimize.constant.budget(totalBudget,readDepth,cellsPerPerson,costKit,
                              costFlowCell,readsPerFlowcell,ct.freq,comp.type,
                              eqtl.ref.study,ref.study.name,personsPerLane,
                              read.umi.fit,gamma.mixed.fits,gamma.probs,
                              ct,disp.fun.param,
                              mappingEfficiency=mappingEfficiency)
