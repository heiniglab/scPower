...
The supplemenatry figure shows how the expected number of expressed/detectable genes depends on the experimental parameters cells per person and read depth. It is an extension of Figure 3, where only the expression probability is shown.
...
Figure S10 is an extension of Figure 3, showing the detection power for all analysed reference studies with PBMC cell types and matched cell type expression model, while in Figure 3 it is only shown for one DE and one eQTL study.
...
Figure S11 shows two example calculations on how scPower selects the optimal experimental parameter combination for a specific budget, once for a DE study and once for an eQTL study. For an explanation of all the used parameters, please look into the introduction vignette.
...
g.rd<-g.rd + 
  scale_x_continuous("Cells per individual",sec.axis=sec_axis(~., breaks=labels,
                                              labels=sapply(labels,function(c)floor(
                                                                      sampleSizeBudgetCalculation.restrictedDoublets(c,max.study$readDepth,
...
g.cp <- g.cp + scale_x_continuous("Read depth",sec.axis=sec_axis(~., breaks=labels,
                                                      labels=sapply(labels,function(c)floor(
                                                        sampleSizeBudgetCalculation.restrictedDoublets(max.study$totalCells,
    c,totalBudget, costKit, cellsPerLane, costFlowCell,readsPerFlowcell))),                                                              name="Sample size"))+
...
g.rd<-g.rd + 
  scale_x_continuous("Cells per individual",sec.axis=sec_axis(~., breaks=labels,
                                                              labels=sapply(labels,function(c)floor(sampleSizeBudgetCalculation.restrictedDoublets(
...
Figure S12 shows the parameter grids for the calculations done for Figure S11.
